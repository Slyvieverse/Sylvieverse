        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-[--color-background-800] rounded-2xl p-6 md:p-8 border border-[--color-primary-700]/50 shadow-2xl shadow-[--color-background-900] transform transition-all duration-300 hover:shadow-[--color-primary-900]/50">
                ...
                <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div class="field-group">
                        ...
                        >
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}" class="bg-[--color-background-800]" {{ old('category_id') == $category->id ? 'selected' : '' }}>
                                    {{ $category->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-6">
                        <div class="field-group">
                            ...
                            <select
                                ...
                                required
                            >
                                <option value="active" class="bg-[--color-background-800]" {{ old('status') === 'active' ? 'selected' : '' }}>Active</option>
                                <option value="inactive" class="bg-[--color-background-800]" {{ old('status') === 'inactive' ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('status')
                                <p class="text-red-400 text-sm mt-2">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="field-group">
                        <label for="image_url" class="block text-sm font-medium text-[--color-text-200] mb-2">Product Image (Upload) <span class="text-red-500">*</span></label>
                        <input
                            type="file"
                            name="image_url"
                            id="image_url"
